<?php
$typeLabels = ['quotation' => 'báo giá', 'order' => 'đơn hàng bán', 'contract' => 'hợp đồng', 'installation' => 'yêu cầu thi công'];
...
$defaultContents = [
    ...
    'installation' => '<p style="text-align:right;font-style:italic">{{today_text}}</p><h2 style="text-align:center">GIẤY YÊU CẦU THI CÔNG</h2><table style="width:100%"><tr><td style="width:60%"><p><strong>Người yêu cầu:</strong> {{requester_name}} - {{requester_phone}}</p><p><strong>Đơn vị thi công:</strong> {{contractor}}</p><p><strong>Địa chỉ lắp đặt:</strong> {{installation_address}}</p><p><strong>Tên khách hàng:</strong> {{customer_name}} - {{customer_contact_name}}: {{customer_contact_phone}}</p><p><strong>Thời gian yêu cầu thi công:</strong> {{requested_date}}</p></td><td style="width:40%;vertical-align:top"><div style="border:1px solid #333;padding:6px"><div><strong>Số CF:</strong> {{cf_number}}</div><div><strong>Mã KH:</strong> {{customer_code}}</div><div><strong>Bộ phận:</strong> {{department}}</div></div></td></tr></table><h3 style="text-align:center">NỘI DUNG YÊU CẦU THI CÔNG</h3><p>{{items_table}}</p><div style="border:1px solid #333;padding:8px;min-height:80px"><p><strong>Người nhận yêu cầu:</strong></p><p>- Thời gian tiến hành thi công: {{execution_date}}</p><p>- Người thi công: {{installer_name}}</p><p>- Báo cáo tình trạng hàng hóa của thợ: {{condition_report}}</p></div>',
];
