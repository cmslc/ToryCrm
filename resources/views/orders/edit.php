<?php
$pageTitle = 'Sửa đơn hàng ' . $order['order_number'];
$o = $order;
$ec = $editContact ?? null;
$ecName = '';
if ($ec) {
...
<div class="page-title-box d-flex align-items-center justify-content-between">
    ...
    <div class="d-flex gap-2">
        <a href="<?= url('orders/' . $o['id']) ?>" class="btn btn-soft-secondary">Quay lại</a>
        <button type="submit" form="orderForm" class="btn btn-primary"><i class="ri-save-line me-1"></i> Cập nhật</button>
    </div>
</div>

<form method="POST" action="<?= url('orders/' . $o['id'] . '/update') ?>" id="orderForm" enctype="multipart/form-data">
    <?= csrf_field() ?>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-2">
                            <div class="flex-grow-1 position-relative">
                                <input type="hidden" name="contact_id" id="contactIdInput" value="<?= $o['contact_id'] ?? '' ?>">
                    ...
                    <div class="row">
                        <div class="col-6 mb-3">
                            ...
                            <div id="qCustomerName" class="form-control-plaintext fw-medium"><?= e($ec['company_name'] ?? ($ec['full_name'] ?? '-')) ?></div>
                        </div>
                        <div class="col-6 mb-3">
                            <label class="form-label">Điện thoại</label>
                            <div id="qPhoneDisplay" class="form-control-plaintext"><?= e($ec['company_phone'] ?? $ec['phone'] ?? '-') ?></div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-6 mb-3">
                            <label class="form-label">Địa chỉ</label>
                            <div id="qAddressDisplay" class="form-control-plaintext"><?= e($ec['address'] ?? '-') ?></div>
                        </div>
                        <div class="col-6 mb-3">
                            <label class="form-label">Email</label>
                            <div id="qEmailDisplay" class="form-control-plaintext"><?= e($ec['company_email'] ?? $ec['email'] ?? '-') ?></div>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="contact_person_id" id="contactPersonSelect">
                            ...
                            <?php if ($o['contact_id']):
                                $cpList = \Core\Database::fetchAll("SELECT id, title, full_name, phone, email, position, is_primary FROM contact_persons WHERE contact_id = ? ORDER BY is_primary DESC, sort_order, id", [$o['contact_id']]);
                                $savedCpId = $o['contact_person_id'] ?? null;
                ...
                <div class="col-lg-6">
                    ...
                    <div class="row">
                        <div class="col-6 mb-3">
                            ...
                            <input type="text" class="form-control" value="<?= e($o['order_number']) ?>" readonly>
                        </div>
                        <div class="col-6 mb-3">
                            <label class="form-label">Ngày đặt hàng</label>
                            <input type="date" class="form-control" name="issued_date" value="<?= e($o['issued_date'] ?? substr($o['created_at'] ?? '', 0, 10)) ?>">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-6 mb-3">
                            ...
                            <select name="owner_id" class="form-select searchable-select" required>
                                ...
                                <optgroup label="<?= e($dept) ?>">
                                    ...
                                    <option value="<?= $u['id'] ?>" data-avatar="<?= e($u['avatar'] ?? '') ?>" <?= ($o['owner_id'] ?? '') == $u['id'] ? 'selected' : '' ?>><?= e($u['name']) ?></option>
                                    <?php endforeach; ?>
                                </optgroup>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="lading_code" value="<?= e($o['lading_code'] ?? '') ?>">
                    </div>
                    <div class="row">
                        <div class="col-6 mb-3">
                        ...
                        <div class="col-6 mb-3">
                            ...
                            <input type="number" class="form-control" name="commission_amount" value="<?= (float)($o['commission_amount'] ?? 0) ?>" min="0">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body" style="background:#e8f0fe">
                    ...
                    <div class="mb-2 d-flex align-items-center justify-content-between">
                        ...
                        <input class="form-check-input" type="checkbox" name="shipping_after_tax" value="1" <?= ($o['shipping_after_tax'] ?? 0) ? 'checked' : '' ?>>
                    </div>
                    <div class="mb-2 d-flex align-items-center gap-2">
                        <span class="flex-shrink-0" style="width:140px">Phí vận chuyển</span>
                        <div class="input-group" style="width:120px"><input type="number" class="form-control" name="transport_percent" value="<?= (float)($o['transport_percent'] ?? 0) ?>" min="0" step="0.01" onchange="calcPaymentRow(this,'transport')"><span class="input-group-text">%</span></div>
                        <input type="number" class="form-control" name="transport_amount" value="<?= (float)($o['transport_amount'] ?? 0) ?>" min="0" onchange="calculateTotal()" style="width:120px">
                    </div>
                    <div class="mb-2 d-flex align-items-center gap-2">
                        <span class="flex-shrink-0" style="width:140px">Chiết khấu</span>
                        <div class="input-group" style="width:120px"><input type="number" class="form-control" name="discount_percent" value="<?= (float)($o['discount_percent'] ?? 0) ?>" min="0" step="0.01" onchange="calcPaymentRow(this,'discount')"><span class="input-group-text">%</span></div>
                        <input type="number" class="form-control" name="discount_amount" value="<?= (float)($o['discount_amount'] ?? 0) ?>" min="0" onchange="calculateTotal()" style="width:120px">
                    </div>
                    <div class="mb-2 d-flex align-items-center justify-content-between">
                        <span>Chiết khấu sau thuế</span>
                        <input class="form-check-input" type="checkbox" name="discount_after_tax" value="1" <?= ($o['discount_after_tax'] ?? 0) ? 'checked' : '' ?>>
                    </div>
                    <div class="mb-2 d-flex align-items-center gap-2">
                        <span class="flex-shrink-0" style="width:140px">Thuế VAT</span>
                        <div class="input-group" style="width:120px"><input type="number" class="form-control" name="tax_rate" value="<?= (float)($o['tax_rate'] ?? 0) ?>" min="0" step="0.01" onchange="calcPaymentRow(this,'tax')"><span class="input-group-text">%</span></div>
                        <input type="number" class="form-control" name="tax_amount" value="<?= (float)($o['tax_amount'] ?? 0) ?>" readonly style="width:120px;background:#dce6f5">
                    </div>
                    <div class="mb-2 d-flex align-items-center gap-2">
                        <span class="flex-shrink-0" style="width:140px">Phí lắp đặt</span>
                        <div class="input-group" style="width:120px"><input type="number" class="form-control" name="installation_percent" value="<?= (float)($o['installation_percent'] ?? 0) ?>" min="0" step="0.01" onchange="calcPaymentRow(this,'installation')"><span class="input-group-text">%</span></div>
                        <input type="number" class="form-control" name="installation_amount" value="<?= (float)($o['installation_amount'] ?? 0) ?>" min="0" onchange="calculateTotal()" style="width:120px">
                    </div>
                    <hr class="my-2">
...
// Persons already rendered by PHP, no AJAX needed on load
...
if (existingItems.length > 0) {
    existingItems.forEach(function(item) {
        addItem({ product_id: item.product_id, product_name: item.product_name, sku: item.product_sku || '', unit: item.unit, quantity: item.quantity, unit_price: item.unit_price, discount: item.discount || 0, tax_rate: item.tax_rate || 0 });
    });
} else {
    addItem();
}
