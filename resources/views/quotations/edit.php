        ...
        fetch('<?= url("products/search-ajax") ?>?q=' + encodeURIComponent(q))
            ...
            .then(results => {
                ...
                drop.innerHTML = results.map(p => {
                    ...
                    return `<div class="pd-item d-flex align-items-center" onclick="pickProduct(${idx}, ${JSON.stringify(p).replace(/"/g, '&quot;')})">
                        ${imgHtml}
                        <div class="flex-grow-1" style="min-width:0">
                            <div><strong>${p.name}</strong> <span class="pd-sku">${p.sku || ''}</span></div>
                            <small class="text-muted">${Number(p.price).toLocaleString('vi-VN')} ₫ / ${p.unit || 'Cái'}</small>
                        </div>
                    </div>`;
                }).join('');
