                            ...
                            <div class="contact-person-item border rounded p-3 mb-3" data-index="0">
                                ...
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    ...
                                    <div class="flex-grow-1">
                                        <div class="row">
                                            ...
                                            <div class="col-7">
                                                <input type="hidden" name="cp_person_id[]" class="cp-person-id" value="">
                                                <input type="text" class="form-control cp-name-input" name="cp_name[]" placeholder="Họ và tên *" required autocomplete="off">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="input-group position-relative">
                                        <input type="text" class="form-control cp-phone-input" name="cp_phone[]" autocomplete="off">
...
// ---- Phase 3: Link existing person when phone matches ----
(function() {
    ...
    function searchPersons(input) {
        ...
        var row = input.closest('.col, .mb-2, [data-cp-row]') || input.closest('.card-body') || input.parentElement;
                ...
                .then(list => {
                    ...
                    list.forEach(function(p) {
                        ...
                        item.addEventListener('click', function(e) {
                            e.preventDefault();
                            var scope = input.closest('[data-cp-scope]') || wrap.closest('.card-body') || document;
                            var root = wrap.closest('.row, .cp-row, .card, .mb-3') || wrap.parentElement.parentElement.parentElement;
                            // Find sibling inputs in same contact person block
    ...
    document.addEventListener('input', function(e) {
        if (e.target.classList.contains('cp-phone-input') || e.target.classList.contains('cp-name-input')) {
            ...
            // User edits → unlink existing person_id (nếu thay đổi thông tin)
            var pidEl = e.target.closest('.row, .cp-row, .card, .mb-3')?.querySelector('.cp-person-id');
            if (pidEl && pidEl.value) {
                // Keep linked — user có thể chỉ sửa chức vụ/note, vẫn cùng person
            }
        }
    });
