<?php $pageTitle = 'Sửa đơn mua ' . $order['order_code']; ?>

        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Sửa đơn mua: <?= e($order['order_code']) ?></h4>
        ...
        <form method="POST" action="<?= url('purchase-orders/' . $order['id'] . '/update') ?>">
            <?= csrf_field() ?>
            <div class="row">
                <div class="col-lg-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" class="form-control" value="<?= e($order['order_code']) ?>" readonly>
                                </div>
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select name="supplier_id" class="form-select">
                                        ...
                                        <?php foreach ($suppliers ?? [] as $s): ?>
                                            <option value="<?= $s['id'] ?>" <?= ($order['supplier_id'] ?? '') == $s['id'] ? 'selected' : '' ?>><?= e($s['name']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Ngày dự kiến nhận</label>
                                    <input type="date" class="form-control" name="expected_date" value="<?= $order['expected_date'] ?? '' ?>">
                                </div>
                                ...
                                <table class="table align-middle mb-0">
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td><input type="number" class="form-control form-control-sm" name="discount_amount" value="<?= $order['discount_amount'] ?? 0 ?>" min="0" onchange="calcTotal()"></td>
                                            <td></td>
                                        </tr>
                                        <tr>
                ...
                <div class="col-lg-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <select name="status" class="form-select">
                                    <?php foreach (['draft'=>'Nháp','pending'=>'Chờ duyệt','approved'=>'Đã duyệt','receiving'=>'Đang nhận','completed'=>'Hoàn thành','cancelled'=>'Đã hủy'] as $k => $v): ?>
                    ...
                    <div class="card">
                        <div class="card-body d-flex gap-2">
                            ...
                            <a href="<?= url('purchase-orders/' . $order['id']) ?>" class="btn btn-soft-secondary">Hủy</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        function addItem(data) {
            ...
            products.forEach(p => {
                opts += `<option value="${p.id}" data-price="${p.price}" data-unit="${p.unit}" data-tax="${p.tax_rate}" ${data && p.id == data.product_id ? 'selected' : ''}>${p.name}${p.sku ? ' ('+p.sku+')' : ''}</option>`;
            });
            ...
            tr.innerHTML = `
                ...
                <td><input type="number" class="form-control form-control-sm" name="items[${i}][quantity]" value="${data ? data.quantity : 1}" min="0.01" step="0.01" onchange="calcRow(${i})"></td>
                <td><input type="text" class="form-control form-control-sm" name="items[${i}][unit]" id="pu-${i}" value="${data ? data.unit : 'Cái'}"></td>
                <td><input type="number" class="form-control form-control-sm" name="items[${i}][unit_price]" id="pp-${i}" value="${data ? data.unit_price : 0}" min="0" onchange="calcRow(${i})"></td>
                <td><input type="number" class="form-control form-control-sm" name="items[${i}][tax_rate]" id="pt-${i}" value="${data ? data.tax_rate : 0}" min="0" max="100" step="0.01" onchange="calcRow(${i})"></td>
