...
class QuotationController extends Controller
{
    ...
    public function index()
    {
        ...
        $quotations = Database::fetchAll(
            "SELECT q.*,
                    ...
                    cp.full_name as contact_person_name, cp.phone as contact_person_phone
             FROM quotations q
             LEFT JOIN contacts c ON q.contact_id = c.id
    ...
    public function export()
    {
        ...
        $columns = [
            ...
            'contact_phone'       => ['label' => 'SĐT liên hệ'],
            'contact_email'       => ['label' => 'Email liên hệ'],
            ...
            'tax_rate'            => ['label' => 'Thuế suất %'],
            'shipping_fee'        => ['label' => 'Phí vận chuyển'],
            'installation_fee'    => ['label' => 'Phí lắp đặt'],
            'total'               => ['label' => 'Tổng'],
            'currency'            => ['label' => 'Tiền tệ'],
            'valid_until'         => ['label' => 'Hiệu lực đến'],
            'sent_at'             => ['label' => 'Ngày gửi'],
            'accepted_at'         => ['label' => 'Ngày chấp nhận'],
            'rejected_at'         => ['label' => 'Ngày từ chối'],
            'owner_name'          => ['label' => 'Phụ trách'],
            'view_count'          => ['label' => 'Lượt xem'],
            'description'         => ['label' => 'Mô tả'],
            'terms'               => ['label' => 'Điều khoản'],
            'notes'               => ['label' => 'Ghi chú'],
    ...
    public function convertToOrder($id)
    {
        ...
        try {
            ...
            $orderId = Database::insert('orders', [
                ...
                'discount_type' => 'fixed',
                'transport_amount' => $quotation['shipping_fee'] ?? 0,
                'installation_amount' => $quotation['installation_fee'] ?? 0,
    ...
    public function pdf($id)
    {
        ...
        if ($templateId) {
            ...
            if ($html) {
                ...
                echo '<style>body{font-family:"DejaVu Sans",Arial,sans-serif;font-size:13px;line-height:1.3;color:#333;padding:15px 30px;margin:0}p{margin:2px 0}table{border-collapse:collapse}td,th{padding:4px 6px}@page{margin:10mm 15mm;size:A4}@media print{body{padding:0}table{page-break-inside:auto}tr{page-break-inside:avoid;break-inside:avoid}thead{display:table-header-group}tfoot{display:table-footer-group}h1,h2,h3,h4{page-break-after:avoid;break-after:avoid}}</style>';
                echo '</head><body>';
                echo $html;
